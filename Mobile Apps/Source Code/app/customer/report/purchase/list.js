...
import { useGetCustomerPurchaseReportQuery } from "../../../../redux/features/report/reportApi";
...
const PurchaseReportList = () => {
  ...
  const height = Dimensions.get("window").height - 480;
  ...
  const renderItem = ({ item }) => (
    <View style={[styles.tableRow, styles.tableBody]}>
      <View style={[styles.tableCol1, styles.tableBodyCol]}>
        <Link href={`/customer/invoice/${item?.id}`}>
      ...
      <View style={styles.tableCol4}>
        <Text preset="h6" style={{ color: colors.pcolor }}>
          {currency_symbol}{" "}
          {(Number(item?.total) - Number(item?.total_paid)).toFixed(2)}
        </Text>
      </View>
    </View>
  );
  ...
  const {
    ...
    error,
    isError,
  } = useGetCustomerPurchaseReportQuery({ fromDate, toDate, warehouse, query });
  ...
  return (
    <>
      <Topbar title="Purchase Report" customer={true} />
        ...
        <View>
          <View style={styles.reportInfoCard}>
            ...
            <Text preset="h3_r" style={[styles.infoItem]}>
              ...
              {purchaseReportData?.data?.gross_total?.toFixed(2) || "0.00"}
            </Text>
            <Text preset="h3_r" style={[styles.infoItem]}>
              Net Total : {currency_symbol}{" "}
              {purchaseReportData?.data?.total_paid?.toFixed(2) || "0.00"}
            </Text>
            <Text preset="h3_r" style={[styles.infoItem]}>
              Due : {currency_symbol}{" "}
              {(
                purchaseReportData?.data?.gross_total -
                purchaseReportData?.data?.total_paid
              ).toFixed(2) || "0.00"}
            </Text>
          </View>
            ...
            <View style={{ height: height }}>
              ...
              {!isLoading && (
                <View style={[styles.tableRow, styles.tableFooter]}>
                  <View style={styles.tableFooterCol1}>
                    <Text preset="h5_m">Total</Text>
                  </View>
                  <View style={styles.tableFooterCol2}>
                    <Text preset="h5_m">
                      {currency_symbol}{" "}
                      {purchaseReportData?.data?.gross_total?.toFixed(2) ||
                        "0.00"}
                    </Text>
                  </View>
                  <View style={styles.tableFooterCol3}>
                    <Text preset="h5_m">
                      {currency_symbol}{" "}
                      {(
                        purchaseReportData?.data?.gross_total -
