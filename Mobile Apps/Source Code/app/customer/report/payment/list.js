...
const PaymentList = () => {
    ...
    <View style={[styles.tableRow, styles.tableBody]}>
      ...
      <View style={[styles.tableCol3, styles.tableBodyCol]}>
        {item?.payment_type == "CASH" && (
          <Text preset="h6" style={{ color: colors.red }}>
            {item?.payment_type}
          </Text>
        )}
        {item?.payment_type == "STRIPE" && (
          <Text preset="h6" style={{ color: colors.blue }}>
            {item?.payment_type}
          </Text>
        )}
        {item?.payment_type == "BANK" && (
          <Text preset="h6" style={{ color: colors.green }}>
            {item?.payment_type}
          </Text>
        )}
      </View>
  ...
  //get data Payment report list
  const {
    data: paymentReportData,
    isLoading,
    error,
    isError,
  } = useGetCustomerPaymentReportQuery({ fromDate, toDate, warehouse, query });
  ...
  } else if (paymentReportData?.data?.data?.length === 0) {
    content = <NotFoundMessage message="Payment Report Not Found" />;
  } else if (paymentReportData?.data?.data?.length > 0) {
    content = (
      <FlatList
        data={paymentReportData?.data?.data}
        ...
        <View>
          <View style={styles.reportInfoCard}>
            <Text preset="h3_r" style={styles.infoItem}>
              Payment Report :{" "}
              {paymentReportData?.data?.report_range || "All Time"}
            </Text>
            <Text preset="h3_r" style={[styles.infoItem, styles.borderNone]}>
              Total : {currency_symbol}{" "}
              {paymentReportData?.data?.total?.toFixed(2) || "0.00"}
            </Text>
          </View>
