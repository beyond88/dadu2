...
import { addItemToInvoice, capitalize } from "../../../../utils/helper";
import {
...
const UpdateInvoice = () => {
  ...
  const { data: singleInvoiceData, isSuccess: singleInvoiceIsSuccess } =
    useGetSingleInvoiceQuery(slug);

  useEffect(() => {
    if (singleInvoiceData) {
      const { data } = singleInvoiceData;
      setSelectCustomer(Number(data?.customer_id));
      setInvoiceItem(
        data?.items?.map((item) => {
          return {
            ...item,
            ...
            cAddedQuantity: item?.quantity,
            givenDiscount: item?.discount,
            givenDiscountType: item?.discount_type,
          };
        })
      );
      setPaymentType(data?.payment_type);
      setTotalPaid(data?.total_paid);
      setAccountNumber(data?.bank_info?.ac_no);
      setTransactionNumber(data?.bank_info?.t_no);
      setNotes(data?.notes);
      setTotalDiscountType(data?.global_discount_type);
      setTotalDiscountValue(data?.global_discount);
    }
  }, [singleInvoiceData, singleInvoiceIsSuccess]);
  ...
  const handleUpdateInvoice = () => {
    ...
    updateAdminInvoice({ data: invoiceData, id: slug });
  };
      ...
      <ScrollView style={{ paddingHorizontal: 20 }}>
        ...
        <View style={{ marginBottom: 90 }}>
            ...
            <View>
              <View>
                ...
                <FormDate
                  ...
                  selectedDate={singleInvoiceData?.data?.date}
                />
              </View>
              <View>
                <Text preset="h3" style={{ marginBottom: 8 }}>
                ...
                <FormDate
                  ...
                  selectedDate={singleInvoiceData?.data?.due_date}
                />
              </View>
            </View>
          </View>
              ...
              <>
                ...
                <View style={{ marginBottom: 10 }}>
                  ...
                  <FormDate
                    ...
                    selectedDate={singleInvoiceData?.data?.bank_info?.date}
                  />
                </View>
              </>
            )}
          ...
          <View style={styles.draftConfirmBtnWrap}>
            <Pressable onPress={() => handleUpdateInvoice()}>
              <LinearGradient
                colors={["#37DBD9", "#008AA1"]}
