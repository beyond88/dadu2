...
const CityList = () => {
  ...
  const handleEditNavigation = (id) => {
    router.push(`/admin/setting/city/edit/${id}`);
  };
  //handle delete
  const [
    deleteCity,
      ...
      >
        <View style={[styles.tableRow, styles.tableBody]}>
          ...
          <View style={[styles.tableCol2, styles.tableBodyCol]}>
            <Text
              preset="h6"
              style={{ color: colors.pcolor, textAlign: "center" }}
            >
              {item?.name}
            </Text>
          </View>
  ...
  const {
    ...
    isError,
    isSuccess,
  } = useGetCitiesListQuery(page);

  const { current_page, to, total } = getCityList?.data?.meta || {};
  // page & data set
  useEffect(() => {
    if (
      isSuccess &&
      Array.isArray(getCityList?.data?.data) &&
      page == current_page
    ) {
      setCityList((prevData) => [...prevData, ...getCityList?.data?.data]);
    }
  }, [isSuccess, getCityList]);
  const fetchMoreData = () => {
  ...
  return (
    <>
      ...
      <Topbar title="City List" />
      <View style={{ paddingHorizontal: 20 }}>
        <Pressable onPress={() => router.push("/admin/setting/city/create")}>
