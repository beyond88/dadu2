...
const StateList = () => {
  ...
  const handleEditNavigation = (id) => {
    router.push(`/admin/setting/state/edit/${id}`);
  };
  //handle delete
  const [
    deleteState,
  ...
  const {
    ...
    isError,
    isSuccess,
  } = useGetStatesListQuery(page);

  const { current_page, to, total } = getStateList?.data?.meta || {};
  // page & data set
  useEffect(() => {
    if (
      isSuccess &&
      Array.isArray(getStateList?.data?.data) &&
      page == current_page
    ) {
      setStateList((prevData) => [...prevData, ...getStateList?.data?.data]);
    }
  }, [isSuccess, getStateList]);
  const fetchMoreData = () => {
  ...
  return (
    <>
      ...
      <Topbar title="State List" />
      <View style={{ paddingHorizontal: 20 }}>
        <Pressable onPress={() => router.push("/admin/setting/state/create")}>
