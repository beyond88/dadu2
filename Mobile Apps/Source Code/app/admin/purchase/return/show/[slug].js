...
import { usePurchaseReturnDetailsQuery } from "../../../../../redux/features/purchase/purchaseApi";
...
const PurchaseReturnDetails = () => {
  ...
  //get purchases data
  const { data: purchaseReturnDetails } = usePurchaseReturnDetailsQuery(slug);
  ...
  const {
    ...
    short_address,
    returned_items,
  } = purchaseReturnDetails?.data || {};

  const total = returned_items?.reduce((acc, item) => {
    return acc + Number(item?.sub_total);
  }, 0);
  return (
    <>
      <Topbar title="Purchase Return Details" />
