...
const PurchaseReturnListPage = () => {
  ...
  const handleNavigation = (id) => {
    router.push(`/admin/purchase/return/show/${id}`);
  };
  //handle delete
            ...
            <View style={styles.left}>
              <View>
                ...
                <Text
                  ...
                  style={{ color: colors.fontColor, marginBottom: 2 }}
                >
                  Total product- {item?.total_returned_product}
                </Text>
              </View>
            </View>
  ...
  const {
    ...
    isError,
    error,
  } = useGetPurchaseReturnListQuery(page);

  const { current_page, to, total } = returnListData?.data?.meta || {};

  // page & data set
  useEffect(() => {
    if (
      isSuccess &&
      Array.isArray(returnListData?.data?.data) &&
      page == current_page
    ) {
      setReturnList((prevData) => [...prevData, ...returnListData?.data?.data]);
    }
  }, [isSuccess, returnListData]);
  ...
  return (
    <>
      ...
      <Topbar title="Purchase Return List" />
