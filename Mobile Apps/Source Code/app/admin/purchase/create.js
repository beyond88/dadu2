...
const PurchaseCreate = () => {
  ...
  const [
    purchaseCreate,
    { data: createdData, isLoading, isSuccess, isError, error },
  ] = usePurchaseCreateMutation();
  const onSubmit = (data) => {
    const createData = {
      ...
      note: data.note,
      product_stock_id: selectedProduct?.map((item) => item?.id),
  ...
  }, [createdData, isSuccess, isError, error]);
