...
const ProductCategory = () => {
  ...
  const handleNavigation = (id) => {
    router.push(`/admin/product/category/edit/${id}`);
  };
  //handle delete
  ...
  const { current_page, to, total } = categories?.data?.meta || {};
  ...
  useEffect(() => {
    if (
      ...
      setCategoryList((prevData) => [...prevData, ...categories?.data?.data]);
    }
  ...
  } else if (categoryList?.data?.data?.length === 0) {
  ...
  const handleCreate = () => {
    router.push("/admin/product/category/create");
  };
  return (
    <>
      {deleteIsLoading && <Loading />}
      <Topbar title="Product Category List" />
