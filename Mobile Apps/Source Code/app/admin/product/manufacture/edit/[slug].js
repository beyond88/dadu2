...
} from "../../../../../redux/features/manufacture/manufactureApi";
...
const ManufactureEdit = () => {
  const [image, setImage] = useState();
  const [fileName, setFileName] = useState(null);
  //get slug
  const { slug } = useLocalSearchParams();
  //get category data
  const { data: singleManufacture, isSuccess: detailsIsSuccess } =
    useDetailManufactureQuery(slug);

  //set form data
  useEffect(() => {
    if (singleManufacture) {
      setValue("name", singleManufacture?.data?.name);
      setValue("desc", singleManufacture?.data?.desc);
      setValue("status", singleManufacture?.data?.status);
      setImage(singleManufacture?.data?.file_url);
    }
  }, [singleManufacture, detailsIsSuccess]);
  //edit category mutation

  const [
    updateManufacture,
    {
      data: updateManufactureData,
  ...
  const onSubmit = (data) => {
    ...
    updateManufacture({ id: slug, body: formData });
  };
  ...
  useEffect(() => {
    if (updateIsSuccess) {
      showMessage({
        message: updateManufactureData?.message,
        type: "success",
      });
  ...
  }, [updateManufactureData, updateIsSuccess, updateIsError, updateError]);

  return (
    <>
      {updateIsLoading && <Loading />}
      <Topbar title="Edit Manufacture" />
