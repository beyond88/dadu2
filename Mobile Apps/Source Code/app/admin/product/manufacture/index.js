...
const BrandList = () => {
  ...
  const handleNavigation = (id) => {
    router.push(`/admin/product/manufacture/edit/${id}`);
  };
  //handle delete
          ...
          <View style={styles.item}>
            <View style={styles.left}>
              ...
              <View style={styles.pContent}>
                ...
                >
                  <Text preset="h5">Manufacture: </Text>
                  {item?.name}
                </Text>
  ...
  const { current_page, to, total } = manufactureData?.data?.meta || {};

  // page & data set
  useEffect(() => {
    if (
      isSuccess &&
      Array.isArray(manufactureData?.data?.data) &&
      page == current_page
    ) {
      setManufactureList((prevData) => [
        ...prevData,
        ...manufactureData?.data?.data,
      ]);
    }
  }, [isSuccess, manufactureData]);
  ...
  } else if (manufactureData?.data?.data?.length === 0) {
    content = <NotFoundMessage message="Category Not Found" />;
  } else if (manufactureList?.length > 0) {
    content = (
      <FlatList
      ...
      <View style={{ marginHorizontal: 20, marginBottom: 180 }}>
        ...
        >
          <Pressable
            ...
            onPress={() => router.push("/admin/product/manufacture/create")}
          >
