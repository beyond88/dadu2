...
const BrandEdit = () => {
  ...
  //get category data
  const { data: singleBrands, isSuccess: detailsIsSuccess } =
    useBrandDetailsQuery(slug);

  //set form data
  useEffect(() => {
    if (singleBrands) {
      setValue("name", singleBrands?.data?.name);
      setValue("desc", singleBrands?.data?.desc);
      setValue("status", singleBrands?.data?.status);
      setImage(singleBrands?.data?.file_url);
    }
  }, [singleBrands, detailsIsSuccess]);
  ...
  const onSubmit = (data) => {
    ...
    updateBrand({ id: slug, body: formData });
  };
  ...
  }, [updateBrandData, updateIsSuccess, updateIsError, updateError]);
          ...
          <View style={styles.formWrap}>
            <View style={styles.formGroup}>
              <Text style={styles.label} preset="h2_sb">
                Brand Name<Text style={{ color: "#ff0000" }}> *</Text>
              </Text>
              <Controller
                control={control}
