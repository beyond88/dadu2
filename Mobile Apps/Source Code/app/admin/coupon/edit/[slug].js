...
const CouponEdit = () => {
  ...
  const { data: getCouponData, isSuccess: getCouponIsSuccess } =
    useShowCouponQuery(slug);

  useEffect(() => {
    if (getCouponIsSuccess && getCouponData) {
      const validityString = getCouponData?.data?.validity;

      // Split the string into start and end dates
      const [startDateString, endDateString] = validityString.split(" - ");
      ...
      const getStartDate = formatDate(startDateString);
      const getEndDate = formatDate(endDateString);
      setValue("title", getCouponData.data.title);
      setValue("code", getCouponData.data.code);
      setValue("discount_type", getCouponData.data.discount_type);
      setValue("discount", getCouponData.data.discount);
      setValue("minimum_shopping", getCouponData.data.minimum_shopping);
      setValue("status", getCouponData.data.status.toLowerCase());
      setSelectedStartDate(getStartDate);
      setSelectedEndDate(getEndDate);
      setImage(getCouponData?.data?.banner_url);
    }
  }, [getCouponIsSuccess, getCouponData]);
  //get form data
  const {
    control,
    handleSubmit,
  ...
  const onSubmit = (data, params) => {
    ...
    updateCoupon({ id: slug, data: formData });
  };
  ...
  }, [updateIsSuccess, updateError, updateIsError, updateData]);
  return (
    <>
