        ...
        return datatables()
            ...
            ->addColumn('action', function ($item) {
                ...
                if (auth()->user()->can('Edit Measurement Unit')) {
                    $buttons .= '<a class="btn btn-sm btn-outline-primary ic-act-btn" href="' . route('admin.measurement-units.edit', $item->id) . '" title="Edit"><i class="mdi mdi-square-edit-outline"></i> ' . __('custom.edit') . ' </a>';
                }
...
';
    ...
    public function query(MeasurementUnit $model)
    {
        ...
        if ($start = request('mu_start_date')) {
            $query->whereDate('created_at', '>=', $start);
        }
        if ($end = request('mu_end_date')) {
    ...
    public function html()
    {
        ...
        return $this->builder()
            ...
            ->minifiedAjax('', "data.mu_start_date=$('#mu_start_date').val();data.mu_end_date=$('#mu_end_date').val();")
