        ...
        return datatables()
            ...
            ->addColumn('action', function ($item) {
                ...
                if (auth()->user()->can('Edit Coupon')) {
                    $buttons .= '<a class="btn btn-sm btn-outline-primary ic-act-btn" href="' . route('admin.coupons.edit', $item->id) . '" title="Edit"><i class="mdi mdi-square-edit-outline"></i> ' . __('custom.edit') . ' </a>';
                }
            ...
            <button class="btn btn-sm btn-outline-danger ic-act-btn delete-list-data"  data-from-name="'. $item->title.'" data-from-id="' . $item->id . '"   type="button" title="Delete"><i class="mdi mdi-trash-can-outline"></i> ' . __('custom.delete') . '</button></form>
            ';
                }
                $buttons .= '<a class="btn btn-sm btn-outline-primary ic-act-btn" href="' . route('admin.coupon.products', $item->id) . '" title="Edit"><i class="flaticon-new-product"></i> ' . __('custom.products') . ' </a>';
...
//                $check      = \Carbon\Carbon::now()->between($startDate,$endDate);
    ...
    public function query(Coupon $model)
    {
        ...
        if ($start = request('cp_start_date')) {
            $query->whereDate('created_at', '>=', $start);
        }
        if ($end = request('cp_end_date')) {
    ...
    public function html()
    {
        ...
        return $this->builder()
            ...
            ->minifiedAjax('', "data.cp_start_date=$('#cp_start_date').val();data.cp_end_date=$('#cp_end_date').val();")
