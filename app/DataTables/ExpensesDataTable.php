        ...
        return datatables()
            ...
            ->addColumn('action', function ($item) {
                ...
                if (auth()->user()->can('Show Expenses')) {
                    $buttons .= '<a class="btn btn-sm btn-outline-info ic-act-btn" href="' . route('admin.expenses.show', $item->id) . '" title="Show"><i class="mdi mdi mdi-desktop-mac"></i> ' . __('custom.show') . ' </a>';
                }
                if (auth()->user()->can('Edit Expenses')) {
                    $buttons .= '<a class="btn btn-sm btn-outline-primary ic-act-btn" href="' . route('admin.expenses.edit', $item->id) . '" title="Edit"><i class="mdi mdi-square-edit-outline"></i> ' . __('custom.edit') . ' </a>';
                }
...
<button class="btn btn-sm btn-outline-danger ic-act-btn delete-list-data" data-from-name="'. $item->title.'" data-from-id="' . $item->id . '"  data-from-id="' . $item->id . '"   type="button" title="Delete"><i class="mdi mdi-trash-can-outline"></i> ' . __('custom.delete') . '</button></form>
';
                }
    ...
    {
        $query = $model->newQuery()->with(['category'])->select('expenses.*');

        if ($start = request('ex_start_date')) {
            $query->whereDate('expenses.date', '>=', $start);
        }
        if ($end = request('ex_end_date')) {
    ...
    public function html()
    {
        ...
        return $this->builder()
            ...
            ->minifiedAjax('', "data.ex_start_date=$('#ex_start_date').val();data.ex_end_date=$('#ex_end_date').val();")
