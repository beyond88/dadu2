    ...
    <div class="col-sm-6">
      <ol class="breadcrumb">
        <li class="breadcrumb-item">
          <a href="#" class="ic-javascriptVoid">{{ __('custom.product_category') }}</a>
        </li>
    ...
    <div class="modal-content">
      ...
      <form action="{{ route('admin.product-categories.import') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          ...
          <a href="{{ static_asset('sample_product_categories_import.xlsx') }}" class="btn btn-link">{{ __('custom.download_sample') }}</a>
        </div>
...
<style>
    ...
    .datepicker.pc-dp table { width: 100%; }
    .datepicker.pc-dp td,
    .datepicker.pc-dp th { width: 40px; height: 36px; font-size: 13px; }
    .datepicker.pc-dp .datepicker-switch,
    .datepicker.pc-dp .prev,
    .datepicker.pc-dp .next { font-size: 14px; padding: 6px; }
</style>
@endpush
...
$(document).ready(function () {
    ...
    $btnBar.append(
        '<span class="pc-date-range" style="display:inline-flex;align-items:center;margin-left:8px;gap:4px;vertical-align:middle;height:100%;">' +
            '<input type="text" id="pc_start_date" class="form-control form-control-sm" placeholder="Start Date" style="width:115px;" autocomplete="off" readonly>' +
            '<span class="text-muted px-1">—</span>' +
            '<input type="text" id="pc_end_date" class="form-control form-control-sm" placeholder="End Date" style="width:115px;" autocomplete="off" readonly>' +
        '</span>'
    );

    $('#pc_start_date').datepicker({ format: 'yyyy-mm-dd', autoclose: true, todayHighlight: true, clearBtn: true, orientation: 'bottom' })
        .on('show', function () { $('.datepicker.dropdown-menu').addClass('pc-dp'); })
        .on('changeDate', function (e) {
            $('#pc_end_date').datepicker('setStartDate', e.date);
            window.LaravelDataTables['dataTableBuilder'].ajax.reload();
        })
        .on('clearDate', function () {
            $('#pc_end_date').datepicker('setStartDate', null);
            window.LaravelDataTables['dataTableBuilder'].ajax.reload();
        });

    $('#pc_end_date').datepicker({ format: 'yyyy-mm-dd', autoclose: true, todayHighlight: true, clearBtn: true, orientation: 'bottom' })
        .on('show', function () { $('.datepicker.dropdown-menu').addClass('pc-dp'); })
