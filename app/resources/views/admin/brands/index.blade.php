    ...
    <div class="modal-content">
      ...
      <form action="{{ route('admin.brands.import') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          ...
          <a href="{{ static_asset('sample_brands_import.xlsx') }}" class="btn btn-link">{{ __('custom.download_sample') }}</a>
...
<style>
    ...
    .datepicker.br-dp table { width: 100%; }
    .datepicker.br-dp td,
    .datepicker.br-dp th { width: 40px; height: 36px; font-size: 13px; }
    .datepicker.br-dp .datepicker-switch,
    .datepicker.br-dp .prev,
    .datepicker.br-dp .next { font-size: 14px; padding: 6px; }
</style>
@endpush
...
$(document).ready(function () {
    ...
    $btnBar.append(
        '<span class="br-date-range" style="display:inline-flex;align-items:center;margin-left:8px;gap:4px;vertical-align:middle;height:100%;">' +
            '<input type="text" id="br_start_date" class="form-control form-control-sm" placeholder="Start Date" style="width:115px;" autocomplete="off" readonly>' +
            '<span class="text-muted px-1">—</span>' +
            '<input type="text" id="br_end_date" class="form-control form-control-sm" placeholder="End Date" style="width:115px;" autocomplete="off" readonly>' +
        '</span>'
    );

    $('#br_start_date').datepicker({ format: 'yyyy-mm-dd', autoclose: true, todayHighlight: true, clearBtn: true, orientation: 'bottom' })
        .on('show', function () { $('.datepicker.dropdown-menu').addClass('br-dp'); })
        .on('changeDate', function (e) {
            $('#br_end_date').datepicker('setStartDate', e.date);
            window.LaravelDataTables['dataTableBuilder'].ajax.reload();
        })
        .on('clearDate', function () {
            $('#br_end_date').datepicker('setStartDate', null);
            window.LaravelDataTables['dataTableBuilder'].ajax.reload();
        });

    $('#br_end_date').datepicker({ format: 'yyyy-mm-dd', autoclose: true, todayHighlight: true, clearBtn: true, orientation: 'bottom' })
        .on('show', function () { $('.datepicker.dropdown-menu').addClass('br-dp'); })
