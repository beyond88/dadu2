                                    ...
                                    @foreach($product_details->allStock->groupBy('warehouse_id') as $warehouseId => $warehouseStocks)
                                        @foreach($warehouseStocks as $stock)
                                            <tr>
                                                ...
                                                <td>{{ currencySymbol() . ' ' . ($product_details->is_variant ? optional($stock->variation)->price : $product_details->price) }}</td>
                                            </tr>
                                        @endforeach
                                    @endforeach
                                    @if($product_details->allStock->isEmpty())
                                        <tr>
                                            <td colspan="{{ $product_details->is_variant ? 8 : 7 }}" class="text-center text-muted">
