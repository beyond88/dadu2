        ...
        <tr v-for="(variant, index) in variants" :key="index">
          ...
          <td>
            ...
            <div class="d-flex align-items-center">
                ...
                />
                <small class="text-danger" v-if="variant.errors.barcode">
                {{ variant.errors.barcode[0] }}
                </small>

                <!-- Barcode Image (move to right, take less space) -->
                <div style="flex: 1; text-align: right;">
                <img
                    class="img-fluid barcode-image barcode-max-height"
                    :id="`barcode-img-${index}`"
                    alt="barcode"
                />
                <input
                    type="hidden"
...
if (this.oldVariants.length) {
  ...
  // Regenerate barcode images for old variants
  this.$nextTick(() => {
    this.variants.forEach((v, i) => {
      this.generateBarcodeImage(i);
    });
  });
  ...
  methods: {
    ...
    generateVariants() {
        ...
        this.variants = combos.map((combo, idx) => {
            ...
            barcode: (Math.floor(Date.now() / 1000) + idx + 1).toString().slice(-10),
            price: "",
        ...
        // Get the generated barcode image (base64)
        const imgEl = document.querySelector(imgId);
        if (imgEl) {
            variant.barcode_image = imgEl.getAttribute("src");
        }
        },
