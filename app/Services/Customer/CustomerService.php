...
class CustomerService extends BaseService
{
    ...
    public function customerShowDetails($customer, $request = null)
    {
        ...
        $toPaySearch   = $request ? $request->get('topay_search', '') : '';
        $invoicePage   = $request ? $request->get('invoice_page', 1) : 1;
        $productPage   = $request ? $request->get('product_page', 1) : 1;
        $toPayPage     = $request ? $request->get('topay_page', 1) : 1;

        $invoices = $this->getInvoicesWithPagination($customer->id, $invoiceSearch, $invoicePage, $productSearch, $productPage, $toPaySearch, $toPayPage);
    ...
    public function getAllCustomerInvoices($customerId, $search = '')
    {
        $query = Invoice::query()
    ...
    public function getAllCustomerNotPaidInvoices($customerId, $search = '')
    {
        $query = Invoice::query()
            ->with(['items.product', 'payments', 'customerInfo'])
            ->latest('date')
    ...
    public function getInvoicesWithPagination($customerId, $search = '', $page = 1, $productSearch = '', $productPage = 1, $toPaySearch = '', $toPayPage = 1)
    {
        $query = Invoice::query()
            ->with(['items.product', 'payments', 'customerInfo'])
            ->latest('date')
        ...
        return $query->paginate(10, ['*'], 'invoice_page', $page)
            ->appends([
                ...
                'topay_search' => $toPaySearch,
                'topay_page' => $toPayPage
            ]);
    }

    public function getNotPaidInvoicesWithPagination($customerId, $search = '', $page = 1, $invoiceSearch = '', $invoicePage = 1, $productSearch = '', $productPage = 1)
    {
        $query = Invoice::query()
            ->with(['items.product', 'payments', 'customerInfo'])
            ->latest('date')
        ...
        return $query->paginate(10, ['*'], 'topay_page', $page)
            ->appends([
                'tab' => 'topay',
                'product_search' => $productSearch,
                'product_page' => $productPage,
    ...
    public function getAllCustomerProducts($customerId, $search = '')
    {
    ...
    public function getProductsWithPagination($customerId, $search = '', $page = 1, $invoiceSearch = '', $invoicePage = 1, $toPaySearch = '', $toPayPage = 1)
    {
        $products = $this->getAllCustomerProducts($customerId, $search);
