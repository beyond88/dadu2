                                    ...
                                    <table class="table table-bordered ic-table-return">
                                        ...
                                        @foreach($purchase->purchaseItems as $key => $purchaseItems)
                                            @php
                                                ...
                                                $rtnLabel = $purchaseItems->product->barrel_label ?: __('custom.barrels');
                                            @endphp
                                            <tr>
                                                <td>{{ $key+1 }}</td>
                                                <td>
                                               ...
                                               <td>
                                                ...
                                                @if($purchaseItems->variation)
                                                    ({{ __("custom.variant") }}: {{ optional($purchaseItems->variation)->name ?? '' }})
                                                @endif
                                            </td>
                                                <td>
                                                <span id="order_quantity_{{ $key+1 }}">{{
                                                    $purchaseItems->receiveItems->sum('quantity') }}</span>@if($purchaseItems->product->is_weight_based)<small class="text-muted">/{{ $rtnLabel }}</small>@endif
                                                </td>
                                                <td class="text-right">{{
                                                currencySymbol().make2decimal(($purchaseItems->price)) }}@if($purchaseItems->product->is_weight_based)<small class="text-muted">/{{ $rtnLabel }}</small>@endif</td>
                                                <td class="text-right">{{
                                                currencySymbol().make2decimal(($purchaseItems->sub_total)) }}</td>

                                                <td class="text-center">{{ $stockQty }}@if($purchaseItems->product->is_weight_based)<small class="text-muted">/{{ $rtnLabel }}</small>@endif</td>
                                                <td class="text-center">{{ $purchaseItems->returnItem->sum('quantity') }}@if($purchaseItems->product->is_weight_based)<small class="text-muted">/{{ $rtnLabel }}</small>@endif
                                                </td>
                                                <td>
                                                    ...
                                                    @if($purchaseItems->product->is_weight_based)
                                                        ...
                                                        <input type="number" step="any" min="0"
                                                               ...
                                                               data-row="{{ $key+1 }}" data-kgpb="{{ $kgpb }}" data-avail="{{ $stockAvailableQty }}"
                                                               data-unit="{{ $rtnLabel }}"
                                                               placeholder="{{ __('custom.weight_kg') }}"
                                                               @if($stockAvailableQty==0) readonly @endif>
                                                        <small class="text-muted d-block">= <span class="ic-wb-barrel" data-row="{{ $key+1 }}">0</span> {{ $rtnLabel }} (1 {{ $rtnLabel }} = {{ $kgpb }} kg)</small>
                                                        <input type="hidden" name="return_quantity[]" class="ic-wb-qty" data-row="{{ $key+1 }}">
                                                    @else
                                                    <input type="number"
                                                ...
                                                <td>
                                                    @if($purchaseItems->product->is_weight_based)
                                                        ...
                                                        <input type="number" step="any" min="0"
                                                               ...
                                                               data-row="{{ $key+1 }}" value="{{ $initPerKg }}"
                                                               @if($stockAvailableQty==0) readonly @endif>
                                                        <small class="text-muted d-block">{{ __('custom.per_kg') }}</small>
                                                        <input type="hidden" name="return_price[]" class="ic-wb-price" data-row="{{ $key+1 }}">
                                                    @else
                                                    <input type="number"
                                                           class="form-control form-control-sm ic-return-calculate-input"
                                                ...
                                                <td>
                                                    @if($purchaseItems->product->is_weight_based)
                                                        <input type="number" readonly
                                                               ...
                                                               data-row="{{ $key+1 }}" name="return_sub_total[]">
                                                    @else
                                                    <input type="number" readonly
...
<script>
    $(function () {
        // Barrel/drum (weight-based) products: the KG entered must resolve to a WHOLE
        // number of barrels/drums. Fractional barrels (e.g. 0.65) are blocked client-side
        // (server-side validation also enforces this).
        function fractionalBarrels($kg) {
        ...
        function refreshRow($kg) {
            var bad  = fractionalBarrels($kg);
            var $td  = $kg.closest('td');
            var $err = $td.find('.wb-int-error');
            $kg.toggleClass('is-invalid', bad);
            if (bad) {
                var unit = $kg.data('unit') || 'barrels';
                if (!$err.length) {
                    $err = $('<small class="wb-int-error d-block error"></small>');
                    $td.append($err);
                }
                $err.text('Quantity must be whole ' + unit + ' (no fractional ' + unit + ').');
            } else {
                $err.remove();
            }
        }

        $(document).on('input change keyup', '.ic-wb-kg', function () { refreshRow($(this)); });

        $('form').on('submit', function (e) {
            var bad = false;
            $('.ic-wb-kg').each(function () {
                if (fractionalBarrels($(this))) { bad = true; refreshRow($(this)); }
            });
            if (bad) {
                e.preventDefault();
                e.stopPropagation();
                alert('One or more return quantities are not whole barrels/drums. Please enter an amount equal to whole units.');
            }
        });
    });
</script>
