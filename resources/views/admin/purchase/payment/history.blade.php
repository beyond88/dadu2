            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h4 class="header-title mb-0">
                        {{ __t('payment_history') }} — #{{ $purchase->purchase_number }}
                    </h4>
                    <div>
                        @if($due > 0)
                        <a href="{{ route('admin.purchases.payment.create', $purchase->id) }}" class="btn btn-sm btn-primary mr-2">
                            <i class="fa fa-plus"></i> {{ __t('make_payment') }}
                        </a>
                        @endif
                        <a href="{{ route('admin.purchases.show', $purchase->id) }}" class="btn btn-sm btn-secondary">
                ...
                @if($payments->isEmpty())
                    <div class="text-center py-5">
                        ...
                        @if($due > 0)
                        <a href="{{ route('admin.purchases.payment.create', $purchase->id) }}" class="btn btn-primary btn-sm">
                            <i class="fa fa-plus"></i> {{ __t('make_payment') }}
                        </a>
                        @endif
                    </div>
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <tfoot>
                            <tr class="table-light font-weight-bold">
                                <td colspan="3" class="text-right">{{ __t('total_paid') }}:</td>
                                <td class="text-right text-success">{{ currencySymbol() }}{{ make2decimal($totalPaid) }}</td>
                                <td colspan="3"></td>
                            </tr>
                            <tr class="table-light font-weight-bold">
                                <td colspan="3" class="text-right">{{ __t('due') }}:</td>
