        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h4 class="header-title mb-0">{{ __t('make_payment') }} — {{ $supplier->full_name }}</h4>
                    <a href="{{ route('admin.suppliers.payment.history', $supplier->id) }}" class="btn btn-sm btn-outline-primary">
                ...
                @if($due <= 0)
                    <div class="alert alert-success">
                        <i class="fa fa-check-circle"></i> {{ __t('fully_paid') ?? 'All purchases for this supplier are fully paid.' }}
                    </div>
                @endif

                <form action="{{ route('admin.suppliers.payment.store', $supplier->id) }}" method="POST">
                    @csrf
                    <div class="row">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <small class="text-muted">Total Due: {{ currencySymbol() }}{{ make2decimal($due) }}</small>
                            @error('amount') <p class="text-danger small mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div class="form-group col-md-12">
                            <label>{{ __t('notes') }}</label>
                            <input type="text" name="notes" class="form-control" value="{{ old('notes') }}" placeholder="Add a note for this bulk payment...">
                    ...
                    <div class="mt-3 d-flex gap-2">
                        ...
                        <a href="{{ route('admin.suppliers.index') }}" class="btn btn-danger ml-2">
