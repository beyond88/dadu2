...
<div class="page-title-box">
    <div class="row align-items-center">
        <div class="col-sm-6">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('admin.suppliers.index') }}">{{ __t('suppliers') }}</a></li>
            ...
            <div class="col-md-4">
                <div class="card border-0 shadow-sm" style="border-left: 4px solid #5664d2 !important;">
                    <div class="card-body py-3">
                        <p class="text-muted mb-1 font-12">{{ __t('total_purchases') }}</p>
                        <h5 class="mb-0 font-weight-bold">{{ currencySymbol() }}{{ make2decimal($totalPurchased) }}</h5>
                    </div>
                </div>
            </div>
            <div class="col-md-4">
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h4 class="header-title mb-0">
                        {{ __t('payment_history') }} — {{ $supplier->full_name }}
                    </h4>
                    <div>
                        @if($due > 0)
                        <a href="{{ route('admin.suppliers.payment.create', $supplier->id) }}" class="btn btn-sm btn-primary mr-2">
                ...
                @if($payments->isEmpty())
                    <div class="text-center py-5">
                        ...
                        <p class="text-muted">No payments recorded for this supplier yet.</p>
                        @if($due > 0)
                        <a href="{{ route('admin.suppliers.payment.create', $supplier->id) }}" class="btn btn-primary btn-sm">
                            <i class="fa fa-plus"></i> {{ __t('make_payment') }}
                        </a>
                        @endif
                    </div>
                    ...
                    <table class="table table-bordered table-hover">
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('admin.purchases.show', $payment->purchase_id) }}">
                                        #{{ optional($payment->purchase)->purchase_number ?? '—' }}
                                    </a>
                                </td>
                                <td>
                                ...
                                <td class="text-center d-print-none">
                                    <form action="{{ route('admin.purchases.payment.destroy', $payment->id) }}" method="POST"
                                          onsubmit="return confirm('Note: Deleting this will not automatically refund the bank account. Are you sure you want to delete this payment?')">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tfoot>
                            <tr class="table-light font-weight-bold">
                                <td colspan="4" class="text-right">{{ __t('total_paid') }}:</td>
