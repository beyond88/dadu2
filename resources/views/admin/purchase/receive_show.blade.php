                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{ $key+1 }}</td>
                                            <td>{{ $receiveItems->purchaseItem->variation ? $receiveItems->purchaseItem->variation->sku : $receiveItems->product->sku }}</td>
                                            <td>
                                                {{ $receiveItems->product->name }}
                                                @if($receiveItems->product->is_variant)
                                                    ({{ __("custom.variant") }}: {{ optional($receiveItems->purchaseItem->variation)->name ?? '' }})
                                                @endif
                                            </td>
                                            <td>{{ $receiveItems->quantity }}@if($receiveItems->product->is_weight_based)<small class="text-muted">/{{ $receiveItems->product->barrel_label ?: __('custom.barrels') }}</small>@endif</td>
                                            <td class="text-right">{{
                                                currencySymbol().make2decimal($receiveItems->price) }}@if($receiveItems->product->is_weight_based)<small class="text-muted">/{{ $receiveItems->product->barrel_label ?: __('custom.barrels') }}</small>@endif</td>
                                            <td class="text-right">{{
